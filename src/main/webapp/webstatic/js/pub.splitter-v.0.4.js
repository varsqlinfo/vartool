...
function setCssCalcWidth(ele, percentVal, pxVal, plusFlag){
	var cssVal ='';
	if(plusFlag !== true){
		cssVal = 'calc('+percentVal+'% - '+pxVal+'px)';
	}else{
		cssVal = 'calc('+percentVal+'% + '+pxVal+'px)';
	}
	
	ele.css('width',cssVal);
	//return percentVal+'%';
}
...
Plugin.prototype ={
	...
	,initPanelSize : function (){
		...
		for(var i =0; i < percentParentEle.length; i++){
			_this._setPercentSize(percentParentEle[i], true);
		}
	...
	,_setInitPanelSize : function (sEle, confPercent, orientationInfo, splitterConf){
		...
		if(orientationInfo=='vertical'){	
			...
			eleTotW-=handleSize; // prev + next  size에서 prev 에서 handle 사이즈 빼주기.

			console.log('_setInitPanelSize', parentWidth, prevW-handleSize, nextW, eleTotW , prevW + nextW)

			_$util.setVerticalWidth(this, sizeInfo, splitterConf, eleTotW, prevW-handleSize, nextW);
		}else{
			...
			if(confPercent){
				resizeEle.css('height', setCssCalcWidth(percent(eleH-handleSize, resizeEle.parent().height()), handleSize));
			}else{
	...
	,_setPercentSize : function (ele, initFlag){
		...
		var freePanelInfo = [];
		var handleSize = 0;//this.options.handleSize;
		var modSize=parentW; 
		var totFreeSize =0; 
		var handleCnt = 0; 
		...
		parentEle.find('.pub-splitter').each(function (){
			...
			if(splitterConf.percent==true){
				...
				var eleW = prevEle.outerWidth();
				var minSize = Math.max(percentage(splitterConf.prevMinSize, parentW), eleW - prevEle.width());

				if(beforeNextEle != prevEle[0]){
					if(minSize >= eleW){
						++handleCnt;
						modSize-= minSize;
						prevEle.css('width', percent(minSize, parentW)+'%');	
					}else{
						...
						totFreeSize +=eleW;
						freePanelInfo.push(splitterConf.prevTarget);
					}
				}else{
					if(panelItemObj[splitterConf.prevTarget]){
				...
				eleW = nextEle.outerWidth()
				minSize = Math.max(percentage(splitterConf.nextMinSize, parentW), eleW - nextEle.width());

				if(minSize >= eleW){
					modSize -= minSize;
					nextEle.css('width', percent(minSize, parentW)+'%');	
				}else{
					...
					totFreeSize += eleW;
					freePanelInfo.push(splitterConf.nextTarget);
				}
		...
		var totMinSize = 0; 
		var calcIgnore = parentW == modSize;
		...
		for(var key in panelItemObj){
			...
			var panelPxSize = percentage(percent(panelItem.eleSize , totFreeSize), modSize); 

			var percentVal = percent(panelPxSize, parentW);
			//console.log(percent(panelItem.eleSize, totFreeSize), percentage(percent(panelItem.eleSize, totFreeSize), modSize), percentVal)
			// handle size 체크해서 처리 하는 로직 구현. 
			// 앞뒤
			// 뒤앞으로 해서 처리. 
			

			if(calcIgnore){
				console.log('aaaaaaa', panelPxSize, percentVal)

				if(panelItem.position == 'prev'){
					setCssCalcWidth(ele, percent(panelItem.eleSize + (!initFlag?handleSize : 0  ) , parentW), handleSize);
				}else{
					ele.css('width', percentVal+'%');	
				}
			}else{

				
				if(panelItem.position == 'prev'){
					var cnt = Math.floor((panelPxSize -panelItem.minSize) / handleSize)-1;

					cnt = cnt > handleCnt ? handleCnt :cnt; 

					console.log('cnt', cnt, panelPxSize , panelItem.minSize)
					 setCssCalcWidth(ele, percentVal, handleSize + (handleSize*cnt));	
					handleCnt -= cnt; 
				}else{
					if(handleCnt > 0){
						setCssCalcWidth(ele, percentVal, (handleSize*handleCnt));	
					}else{
						ele.css('width', percentVal+'%');	
					}
			...
			//console.log(ele, panelItem);
		}
		parentEle.css('min-width',totMinSize+(Object.keys(panelItemObj).length * handleSize)+'px');
	}
	/**
