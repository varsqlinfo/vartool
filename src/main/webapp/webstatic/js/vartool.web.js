...
var _$base = {
	version:'0.1'
	,author:'ytkim'
	,contextPath: (typeof global_page_context_path === 'undefined' ? '/vtool' : global_page_context_path)
	,uri:{
		'manager':'/mgmt'
		,'admin':'/admin'
		,'login':'/login'
...
function setTheme(theme){
	...
	$('html').attr('pub-theme',theme);
	$('html').addClass('pub-theme-'+theme);
}

var themePrefix = 'vartool-theme-';
_$base.theme =function (uid, theme){
	var themeKey = themePrefix+uid; 
	
	if(theme===true){
		var theme = _$base.localStorage(themeKey);
		setTheme(theme);
		return theme; 
	}
	
	if(isUndefined(theme)){
		return _$base.localStorage(themeKey);
	}else if(isString(theme)){
		_$base.localStorage({key: themeKey, value: theme});
		setTheme(theme);
	}
}
/**
...
_$base.req ={
	...
	,ajax:function (option){
		...
		ajaxOpt.success =  function (data, status, jqXHR) {
			...
			try{
				...
				if(option.error){
					option.error.call(this, jqXHR, e);
				}else{
					throw e; 
				}
			}
	...
	,download :function (opts){
		...
		if(_$base.isUndefined(opts.url) && !_$base.isUndefined(FileSaver)){
			var fileInfo = opts.params||{};
			var blob = new Blob([fileInfo.content] ,{type:"text/plain;charset=utf-8"})
			saveAs(blob , fileInfo.fileName||'empty-file-name');
			return ;
		}
		...
		if($('#vartool_hidden_down_area').length < 1){
			var strHtm = '<div id="vartool_hidden_down_area"style="display:none;">'
				+'<iframe name="vartool_hidden_down_iframe"  style="width:0px;height:0px;"></iframe><div id="vartool_hidden_down_form_area"></div><div>';
			$('body').append(strHtm);
		}
		...
		$('#vartool_hidden_down_form_area').empty().html(contHtm.join(''));

		document.vartool_hidden_down_form.submit();
...
_$base.logout = function (callback){
	if(_$base.isFunction(callback)){
		_$base.req.ajax({
			url :{type : 'ignore', url : $vartoolConfig.logoutUrl }
			,success: callback
		})
		return ;
	}

	locatiion.href=$vartoolConfig.logoutUrl;
}
...
_$base.getFileSizePerFile= function (){
	return $vartoolConfig.file.sizePerFile || 1000;
}
...
_$base.unload =function (mode){
	...
	$(document).keydown(function (e) {
	    ...
	    if(keychk){
			...
			if(!confirm(_$base.messageFormat('vartool.0001'))){
				return false;
			}else{
				e.preventDefault()
                e.stopPropagation()
...
function cloneDeep(dst, src) {
	if(VARTOOL.isObject(src)){
		return cloneObjectDeep(dst, src);
	}else if(VARTOOL.isArray(src)){
		return cloneArrayDeep(dst, src);
	}else{
		if (VARTOOL.isDate(src)){
...
function cloneObjectDeep(dst, src) {
	...
	for (let key in src) {
		...
		if ( typeof val !== 'object' || val=== null) {
			...
			dst[key] = cloneDeep(VARTOOL.isArray(val) ? [] : {}, val);
		} else {
...
function cloneArrayDeep(dst, src) {
	var isObj = VARTOOL.isObject(dst);
	...
	for (var i = 0; i < src.length; i++) {
		...
		if(val == null){
			...
			newVal= cloneDeep(VARTOOL.isArray(val) ? [] : {}, val);
		}
...
_$base.util = {
	...
	,renderHtml : function (template, renderItem){
		return VARTOOLTemplate.render.html(template, renderItem);
	}
	/**
	 * @method VARTOOL.util.dateFormat
	 * @param date
	 * @param masks
	...
	/**
     * @method VARTOOL.util.removeSpecial
     * @param str
     * @description 특수문자 제거
     */
	...
	/**
     * @method VARTOOL.util.setRangeDate
     * @param sdtObj
     * @param edtObj
     * @param cdt
     * @param rangeNum
