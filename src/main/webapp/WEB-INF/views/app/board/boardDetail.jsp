...
VartoolAPP.vueServiceBean({
	...
	,methods:{
		...
		,modifyInfo : function (){
			location.href=PubEP.util.replaceParamUrl('<vartool:boardUrl addUrl="modify?articleId=#articleId#"/>',this.articleInfo);
		}
		,download : function (item){
			//console.log(PubEP.util.replaceParamUrl('<vartool:boardUrl addUrl="file?articleId=#article#&fileId=#fileId#"/>',item) , item)
			VARTOOL.req.download({
				url : PubEP.util.replaceParamUrl('<vartool:boardUrl addUrl="file?articleId=#fileContId#&fileId=#fileId#"/>',item)
			});
		}
		,deleteItem : function(){
		...
		,commentSave : function (mode, subMode){
			...
			if(mode == 're'){ // 대댓글
				if(subMode == 'modify'){ //수정
					saveInfo = VARTOOL.util.objectMerge(this.reModifyCommentInfo); 
			...
			if(VARTOOL.isBlank(saveInfo.contents)){
		...
		,commentList : function (){
			...
			this.$ajax({
				url: '<vartool:boardUrl addUrl="commentList" contextPath="false"/>' 
				,data: param
				,success: function(resData) {
		...
		,commentModify : function(item){
			...
			if(VARTOOL.isUndefined(item.modifyContents)){
		...
		,commentDelete : function(item){
			...
			this.$ajax({
				url: '<vartool:boardUrl addUrl="commentDelete" contextPath="false"/>' 
				,data: param
				,method :'delete'
				,success: function(resData) {
					_this.commentList();
